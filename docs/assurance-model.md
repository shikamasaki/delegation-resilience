...
`handover`または`human_takeover`を実証する場合、集約booleansだけでは足りません。Attestationはparticipant ID、role、simulatedか、qualification artifact、participantごとのauthority artifactとoperational-access artifactを保持します。宣言人数とparticipant listが一致し、scenarioで要求された人数以上の実在・qualified participantが揃わなければ実証扱いにしません。

v0alpha2はDSSE、外部trust policy、issuer scope、失効を検証し、human drill preflightが信頼済みartifactを開始条件として使える入口を提供します。Participant本人のacknowledgement/withdrawal statusはrun、challenge、profile、preflight context、briefing、consumer-held sequence high-watermarkへ束縛します。それでも署名は観測内容の真実や自由意思を単独では証明せず、consumer-held high-watermarkも世界全体の最新状態を自動発見しません。Completed drillのobserver、handover/task outcome、external reconciliationを同じrunへ束縛するpredicateが未完成であるため、`handover`と`human_takeover`の能力昇格は引き続きfail closedです。
